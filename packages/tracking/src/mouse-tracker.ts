...
const DEFAULT_DEPTH_STRATEGY: Required<MouseTrackerOptions>['depthStrategy'] = 'radial'
...
type PointerEventLike = PointerEvent | MouseEvent

export class MouseTracker {
  private readonly listeners: Set<FaceTrackingListener>
  private readonly config: TrackerConfig
  private running: boolean
  private usingPointerEvents: boolean
  ...
  private readonly onPointerMove = (event: Event): void => {
    ...
    const metric: FaceTrackingMetric = {
  ...
  private computeDepth(normalizedX: number, normalizedY: number): number {
...
function resolveConfig(options: MouseTrackerOptions): TrackerConfig {
...
function resolvePosition(event: PointerEventLike, element: HTMLElement | Window) {
...
function inferDefaultElement(): HTMLElement | Window {
